...
Checks for sensor drift, wire disconnection, and I2C failures. Ensures data reliability before processing.
...
Reads raw temperature and humidity from dual DHT20 sensors via I2C.
...
Implements the VTT Mathematical Model (C code) to calculate Mold Index (0-6) based on temp/humidity history.
...
Triggers immediate alerts if Mold Index > 1.0 or if critical hardware failure occurs (Hard Real-Time).
...
Transmits JSON payloads via OpenThread Mesh to the Server Node/Gateway.
...
Monitor: nrfjprog --rtt (View System Health Logs)

Project by Muhammad Muzamil (Embedded Systems Architect)